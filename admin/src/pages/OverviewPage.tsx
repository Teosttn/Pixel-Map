...
type Overview = { counts: Record<string, number>; latestDigest?: string; todayDigest?: boolean; branch?: string; remote?: string; changedFiles?: string[] };
...
export function OverviewPage() {
  ...
  return <section className="admin-page"><header className="page-heading"><div><p className="kicker">Workspace</p><h1>Overview</h1></div></header><div className="overview-metrics">{rows.map(([label, count]) => <div key={String(label)}><strong>{count}</strong><span>{label}</span></div>)}</div><div className="overview-grid"><section><h2>内容状态</h2><dl className="definition-list"><div><dt>最近资讯</dt><dd>{overview.latestDigest || "无"}</dd></div><div><dt>今日摘要</dt><dd>{overview.todayDigest ? "已存在" : "尚未生成"}</dd></div></dl></section><section><h2>仓库</h2><dl className="definition-list"><div><dt>分支</dt><dd>{overview.branch || "未知"}</dd></div><div><dt>远端</dt><dd className="truncate">{overview.remote || "未配置"}</dd></div><div><dt>已变更文件</dt><dd>{overview.changedFiles?.length || 0}</dd></div></dl></section></div></section>;
}
